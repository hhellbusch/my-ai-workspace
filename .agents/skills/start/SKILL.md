---
name: start
description: Orient a new session — load project context, check handoffs, and suggest
  what to work on
...
- Handoff file: `ls .planning/<project>/whats-next.md (resolved via BRIEF.md mtime) 2>/dev/null || echo "No handoff file"`
...
Check project-scoped handoffs: the extension now writes checkpoints to `.planning/<project>/whats-next.md` (resolved via `.planning/*/BRIEF.md` mtime). To find the active handoff:

1. Run `find .planning -maxdepth 2 -name whats-next.md -printf '%T@ %p\n' 2>/dev/null | sort -rn | head -5` — lists handoff files sorted by modification time
2. If a handoff exists in a project directory (e.g., `.planning/zanshin-kit/whats-next.md`), use that directory's `BRIEF.md` one-liner to identify the project
3. If multiple handoffs exist, present the most recent one and note others: "There are handoffs from previous sessions for [X], [Y]. Picking up the most recent."
4. Check staleness: run `stat -c %Y .planning/<project>/whats-next.md` and compare against the timestamp of the most recent commit (`git log -1 --format=%ct`). If the handoff is older than the most recent commit, flag it:
...
5. Read it in full
6. Present a summary: what was being worked on, what remains, any blockers or decisions pending
7. Cross-reference against the backlog snapshot from Step 1. If the handoff references work that's no longer in the backlog (completed items are deleted, not archived), or if the backlog has changed significantly, note the discrepancy.
8. Ask: "There's a handoff from a previous session. Want to pick up where you left off, or start fresh from the backlog?"

If no handoff exists in any project directory (crash, abrupt end, or `/whats-next` was skipped):
