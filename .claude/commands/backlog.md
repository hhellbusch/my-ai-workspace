...
1. Read `BACKLOG.md`
2. Count items in each section (In Progress, Up Next, Ideas)
...
3. Add the item to the end of the **Ideas** section
...
5. **Delete the item entirely** from `BACKLOG.md` — no Done section, no archive, no strikethrough. Git history is the record.
6. Update the `> State:` header line with new counts and today's date
7. Update the `Last updated` date
8. Confirm: "Removed: <title> (git history has the record)"
...
`BACKLOG.md` has three sections in this order:
