...
- **`private/`** (if it exists) — A local-only git repo for content that should not appear in the public repo. **Only read or reference `private/` when the user explicitly asks to work privately.** Do not auto-check `private/` during `/start`, `/checkpoint`, `/whats-next`, or any other command. When working privately: commits go to `git -C private/`, handoffs go to `private/.planning/whats-next.md`, public `BACKLOG.md` is not updated, and nothing in `private/` is referenced in public commit messages or file content. See `private/README.md` for full conventions.
