...
**Submodule push discipline:** Submodules are normally in detached HEAD state — git checks out the pinned commit, not a branch. Before committing changes inside a submodule, always check out a named branch first (`git checkout <branch>` or `git checkout -b <branch>`). Never commit in detached HEAD state — the commit will be orphaned when the submodule is next updated. To push all submodules and the parent repo correctly, use `scripts/push-all.sh` — it verifies each submodule's commit is reachable from a remote branch, infers the correct push target from `.gitmodules` or remote refs, and warns rather than guessing when the target is ambiguous.
...
**`git-projects/` — external repo cache (gitignored).** Open-source projects cloned locally for context, exploration, and contribution work. Before fetching docs or source from the web, run `ls git-projects/` — the project may already be cloned. After fetching from the web for a project that has a git repo, offer to clone it here for future sessions. After cloning, update `git-projects/README.md` with a row in the Current Projects table (repo, upstream URL, status, notes). Current contents: `pi-mono`, `paude-proxy`, `ramalama`.
