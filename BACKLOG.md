...
- **Context:** Deep dive drafts complete. D1 navigation hub wired (devops/README, ORGANIZATION, AGENTS, worksheet). **Next:** author review on PR #9; deck PDF diagrams; optional SLO guide; D2 frontmatter after review. Index: [maturity/README.md](../docs/ai-engineering/maturity/README.md) · handoff: [.planning/software-systems-maturity/whats-next.md](../.planning/software-systems-maturity/whats-next.md).
...
### paude-proxy thinking relay — surface Qwen3 reasoning in Pi UI
- **Product:** paude / paude-proxy
- **Context:** LiteLLM strips `reasoning_content` from streaming responses (see adjacent backlog item). Workaround: modify paude-proxy (Go, `git-projects/paude-proxy/`) to intercept POST `/v1/chat/completions` when `enable_thinking: true` or `chat_template_kwargs.enable_thinking: true` is in the body, issue a non-streaming request internally, then re-emit synthetic SSE chunks: first `delta.reasoning_content` chunks (which Pi's stream parser at `openai-completions.ts:258–299` already handles), then regular `delta.content` chunks, then `[DONE]`. Pi's thinking panel renders correctly with no extension changes.
- **Trade-off:** First token arrives only after the full non-streaming response completes — latency penalty equals the model's total thinking + generation time. Gate it behind a flag or only apply when thinking is explicitly enabled to avoid impacting normal requests.
- **Blocked by:** LiteLLM upstream fix (PR #24010). Once that lands and RHOAI upgrades, this workaround becomes unnecessary. Build only if the upstream fix stalls.
- **Added:** 2026-05-12
...
- **Links:** `docs/case-studies/when-the-repository-becomes-the-resume.md`, `ABOUT.md`, `.cursor/rules/cross-linking.mdc`
- **Added:** 2026-04-20
...
- **Context:** When an AI assistant synthesizes from a pre-selected corpus (e.g. NotebookLM fed Red Hat docs + Lenovo whitepapers), the output is fluent and internally consistent but structurally oriented by what the curator chose to include. The bias isn't in what the model does with the sources — it's in what sources were *never* in the room. The model can't notice an absence. The Jared Burck article is the triggering instance: architecturally accurate (the corpus had good architecture docs), economically optimistic (the corpus had vendor marketing but not the Braincuber counter-argument), maturity-blind (the corpus mixed GA and Tech Preview docs without that distinction). **Distinct from** [case study 9](docs/case-studies/context-stripped-citations.md) (*When the Source Says the Opposite of the Claim*) — that case has the source present but the conclusion stripped; this case has the counter-evidence absent from the start. See also: `research/openshift-ai-llm-deployment/assessment.md` (Finding 2, Finding 3 as exemplars).
  - **Self-referential concern:** This workspace is also a curated corpus. The library is hand-selected. Research workspaces fetch sources from articles that already have a POV. AI enriches what's present. If the initial selection is advocacy-skewed, enrichment amplifies it invisibly. The [`shoshin` principles](AGENTS.md) (Session Awareness / Shoshin sections) and [`/spar`](submodules/zanshin-pi-extension/skills/spar/SKILL.md) command are partial mitigations — but they operate *within* the corpus, not on its composition. The honest question for any research or essay here: **what did we not include, and why?** The case study should surface that question as a named practice, not just a risk.
...
- **Links:** `docs/case-studies/context-stripped-citations.md`, `research/openshift-ai-llm-deployment/assessment.md`, `AGENTS.md` (Session Awareness / Shoshin sections), `submodules/zanshin-pi-extension/skills/spar/SKILL.md`
- **Added:** 2026-04-20
