...
Create a directory under `apps/` for each application:
...
- name: Generate directory list for Helm values
  run: |
    APP_DIRS_PATH="./my-custom-apps-path"  # ← Change this
```
...
Edit `charts/argocd-apps/templates/app-of-apps.yaml` to:
...
- Ensure the `apps/` directory exists and contains subdirectories
