...
review:
  status: unreviewed
  notes: "AI-generated 2026-09-09. QA companion to AAP SDLC draft — tools, local/agent loop, CI. Not a substitute for a team's DoD."
---
...
> **Purpose:** Map “quality” onto the [three artifacts](README.md) (content, CaC, payload), name the Ansible tools that actually exist, and show how a workspace and an agent use them without inventing a second toolchain.

This is not a certification syllabus.
It is a picture of **evidence before promotion** — the same idea as Ambler’s sandboxes: cheaper checks first, blast radius contained, fixes go back to development.
...
It is a habit of asking, before the change moves to the next sandbox:

1. **Did we say it clearly?** YAML parses; names match; lint agrees with the team’s profile.
...
**A quality gate** is a check that must pass before the change is allowed into the next sandbox.
Inner loop = you (or an agent) run it in seconds.
Outer loop = CI runs the **same commands** on every merge request so skipped hooks still fail.

False confidence is the usual failure: `ansible-playbook --syntax-check` green does not mean “add worker” works on OpenShift.
...
Molecule is **integration** testing of roles/playbooks, not unit testing.
`ansible-test units` is unit testing of **plugin Python**.
Most AAP playbook shops live in the top and middle of this list; they still need a lab gate before a content tag is pinable in CaC.
...
The [workspaces article](https://developers.redhat.com/articles/2026/08/21/red-hat-ansible-development-workspaces) is the same tools in a browser; it is optional.

**Content repo vs CaC repo:** lint playbooks and roles in the playbook repo.
In the CaC repo, lint the *apply* playbooks; yamllint the object lists; do not pretend Molecule on `controller_templates:` YAML tests a cluster.
...
1. Pin tools (`requirements-qa.txt` or an EE) — same `ansible-lint` profile as CI.
2. `pre-commit install` so a commit without lint does not feel successful.
3. A short script or Makefile target: `make qa` → yamllint + ansible-lint + syntax-check (+ molecule if the role has a scenario).
4. Optional: `ansible-navigator run` so you are not testing against laptop Ansible while AAP uses `ee-supported`.

Crawl: lint + syntax-check on every change.
Walk: Molecule on roles that target a VM/container-shaped OS.
...
CI is the backstop.
The workspace is how you **stop wasting the backstop**.
...
- **Molecule** when the role has a `molecule/` directory — not “write a full cluster scenario” as busywork.
- **Never** `git commit --no-verify` unless the human asked.
- **Never** apply CaC or launch an AAP job against integration/production unless the human asked. Develop AAP + lab is already a promotion.
...
If the agent uses a different linter version than CI, you will get “works on my agent” — the same bug as “works on my machine.”

Optional hardening: a stop/pre-commit hook that fails if `ansible-lint` was not run in the session (this Field Notes repo does that for markdown; a playbook repo can do it for `*.yml` under `playbooks/` and `roles/`).
...
Do not lint **only the diff** and skip the rest of the playbook tree if your roles include each other — that pattern hides breakages ([AOP review](../../ocp/ibm-z/ansible-openshift-provisioning-review.md) hit this).
...
2. Add `pre-commit` + a GitHub **or** GitLab job that runs `ansible-lint` on the playbook tree.
...
1. Which **ansible-lint profile** is realistic for existing product playbooks — `basic` / `moderate` / `production`?
