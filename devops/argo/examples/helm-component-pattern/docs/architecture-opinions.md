...
*Cost:* no automatic cluster discovery — every cluster must be explicitly registered in `clusters.yaml`. See `hub/option-b-applicationset.yaml` for what an ApplicationSet bootstrap would look like if you want it.
