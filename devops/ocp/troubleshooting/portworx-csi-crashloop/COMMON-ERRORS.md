...
```
Error: forbidden: User "system:serviceaccount:portworx:px-account" cannot create resource "csinodes" in API group "storage.k8s.io" at the cluster scope
Error: forbidden: User "system:serviceaccount:portworx:px-account" cannot list resource "nodes" in API group "" at the cluster scope
```

(On legacy installs the namespace in the message may be `kube-system` instead of `portworx`.)
...
oc logs -n $PX_NS -l name=portworx-operator --tail=100
```
...
# To edit deployment:
oc get pod -n $PX_NS $PX_CSI_POD -o jsonpath='{.metadata.ownerReferences[0].name}'
...
oc logs -n $PX_NS $PX_CSI_POD --previous | tail -20
...
oc get svc -n $PX_NS | grep portworx
...
oc get pods -n $PX_NS -l name=portworx-operator -o jsonpath='{.items[0].spec.containers[0].image}'
...
PX_POD=$(oc get pods -n $PX_NS -l name=portworx -o jsonpath='{.items[0].metadata.name}')
oc exec -n $PX_NS $PX_POD -- /opt/pwx/bin/pxctl license list
