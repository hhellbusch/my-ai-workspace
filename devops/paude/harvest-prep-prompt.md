# Paude Harvest Prep Prompt

Paste into a paude agent session when work is done.
Canonical source: `.agents/skills/paude-spec/templates/harvest-prep-prompt.md` (keep in sync).

Replace `<session-name>` and `<short-task-slug>` before sending.
