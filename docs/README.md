...
**Came here via JBGE, TAGRI, or AI doc/epic bloat:**
Start with [Artifact Discipline and AI](ai-engineering/artifact-discipline-and-ai.md) — Scott Ambler's agile modeling ideas applied to AI over-production: who reads the doc, what decision it enables, when to stop writing. Complements spar/shoshin; includes copy-paste prompts for peers.
...
- `essay` [Prompting Is Necessary but Not Sufficient](ai-engineering/prompting-and-state.md) — Prompting skill determines quality within a session. State management determines whether sessions add up to coherent long-horizon work. For work that spans days or weeks, both are needed and neither substitutes for the other. Names the two failure modes, why they're independent, and why they compound when addressed together.
...
- `essay` [LangChain vs. Behavioral Skills — Two Layers, One Mistake](ai-engineering/frameworks-vs-behavior.md) — Frameworks and skills are not alternatives — they solve different layers of the same problem. Names the category error where they're treated as competing options, explains what each layer does, and provides a decision rule for when each is appropriate.
...
- `guide` [OpenShift GPU Node Management](ai-engineering/openshift-gpu-node-management.md) — Bare-metal guide for managing GPU nodes on OpenShift: installation order, ClusterPolicy configuration, the driver upgrade state machine, vGPU for VM passthrough, and troubleshooting. Sources pulled from NVIDIA GPU Operator docs, OpenShift docs, and operator source code (v24.9.1).
...
**Companion guides:**
- `guide` [A Portable AI Toolkit — My Agent, Anywhere](ai-engineering/portable-ai-toolkit.md) — How Paude + Pi + Zanshin form a portable AI agent toolkit: containerize the runtime, containerize the working discipline, push your accumulated context in as the workspace. Explains the three-layer architecture and why each layer matters. Entry point for sharing the overall system.
- `guide` [Getting Started with Paude — Autonomous Agent Sessions](ai-engineering/paude-getting-started.md) — Practical manual for running AI agents in isolated containers: install, session lifecycle, task spec patterns, fire-and-forget workflow, multi-agent comparison. Written from hands-on exploration.
...
- `guide` [Zanshin — Portable Session Context](ai-engineering/framework-bootstrap.md) — Single-file entry point for loading the Zanshin framework into any AI tool. Minimum viable load (four sentences), full behavioral dispositions, and tool-specific instructions for Copilot, Claude Code, and chat-based AI. Clone the repo alongside a project and point your tool at this file.
...
- `guide` [Mobbing — One Keyboard, a Shared Clock](ai-engineering/mobbing.md) — Whole-team work at one keyboard: driver/navigator rotation, a session goal, and a shared timer URL. Calls out [mobti.me](https://mobti.me/) for remote groups. Human–human practice; composes with spar/shoshin when an AI is in the room.
- `guide` [Artifact Discipline and AI — What Agile Modeling Teaches About Working With Assistants](ai-engineering/artifact-discipline-and-ai.md) — JBGE, TAGRI, travel light, document late — Ambler's artifact economics applied to AI doc/plan over-production. Prompts peers can use without this repo.
...
- `guide` [The Case for Local: Disk Management as a Privacy-First AI Task](ai-engineering/local-llm-sysadmin.md) — A case study of using an AI assistant to diagnose and plan disk space cleanup. Covers why filesystem data is private by nature, the iterative analysis loop, and what a recurring local disk agent would look like. The session was conducted with a cloud model — which sharpens the argument rather than softening it.
- `guide` [Drop a YouTube Link, Get a Structured Analysis](ai-engineering/youtube-video-analysis.md) — Explains a workflow for giving an AI a YouTube link and getting back research notes that follow source documentation methodology. Covers what the workflow produces, its limits, and how it fits into the research track.
...
- `philosophy` [Zanshin — What Remains When the Session Ends](philosophy/zanshin.md) — The karate concept of remaining mind explains the hardest problem in AI-assisted work: what survives the session boundary. What the dojo teaches about holding awareness after the technique, why AI tools have no zanshin by design, and what the framework does to compensate. *(Bridge between the philosophy and AI-engineering tracks.)*
...
- `failure` [When the AI Diagnoses Itself](case-studies/ai-self-diagnosis-symptom-report.md) — Copilot's structured self-evaluation of a working-style document was honest, typed, and wrong about the root cause. Context drift was diagnosed as multi-context collision. Documents why AI self-diagnosis is structurally unreliable and why verification discipline must apply to the feedback loop itself.
...
- `failure` [When the Directory Moves and the Links Don't Know](case-studies/link-depth-drift.md) — The same reorganization silently broke 23 internal navigation links — all off by exactly one `../`, all in the moved subtree, none signaled at commit time. Discovered months later by audit. Companion to the gitignore drift case: same root cause, different failure surface. One failure was loud; this one was silent and slow to accumulate.
...
- `failure` [When the Repository Becomes the Resume](case-studies/when-the-repository-becomes-the-resume.md) — The AI consistently misidentified the workspace owner's professional domain from the dominant technical corpus. Names corpus-to-identity conflation: treating what someone has built so far as who they are. Fix: identity-first meta-documents that explicitly decoupled from current content.
...
- `workflow` [When Sparring Finds the Assumption, Not Just the Bug](case-studies/spar-finds-the-assumption.md) — Two different things a spar can catch: implementation errors and unexamined assumptions. The second is harder to find and more valuable — the assumption goes unnoticed precisely because it felt load-bearing.
- `workflow` [Diagnosing Through the Stack — Audio Routing and the WirePlumber Priority Queue](case-studies/diagnosing-through-the-stack.md) — A USB microphone silently rerouted all audio output. Reading four system layers (ALSA → PipeWire → WirePlumber → state file) found the root cause in WirePlumber's priority stack. Stopping at the process layer gives you the command; stopping at the state layer gives you confidence.
