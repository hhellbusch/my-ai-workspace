...
- `guide` [Running a Local LLM: Setup, Tradeoffs, and Real Electricity Cost](ai-engineering/local-llm-setup.md) — Why you might want to run AI locally, what it takes, how to set it up with Cursor and Claude Code (Ollama and RamaLama), and how to design circuit-level electricity measurement. Includes hands-on verified commands for AMD RX 7900 XT / Fedora. Direction-reviewed — setup steps are a starting point, not a finished how-to.
- `guide` [vLLM Reference: Server-Grade Local Inference](ai-engineering/local-llm-vllm.md) — Install, serve, and configure vLLM for maximum throughput and an OpenAI-compatible endpoint. Covers CUDA and ROCm paths. Companion to the main local LLM setup guide — start there if you haven't yet.
