...
**Session orientation that checks for drift.** A [`/start`](../../.agents/skills/start/SKILL.md) command that reads the backlog, checks recent git activity, and suggests focus options gives a new session structure. Adding a [fresh-eyes check](../../.cursor/rules/shoshin.mdc) — comparing project brief goals against current backlog items — catches scope drift that accumulates across sessions without anyone noticing.
