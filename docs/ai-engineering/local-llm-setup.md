...
| `qwen2.5:32b` (general) | 32B dense Q4_K_M | **20 GB (tight)** | Confirmed working on RX 7900 XT with 382 MiB free after load: 18,508 MiB weights + 1,024 MiB KV (n_parallel=4, n_ctx=4096) + 307 MiB compute = 19,839 MiB total. **Requires clean boot** — VRAM fragmentation from prior failed loads causes OOM. Graph splits: 2 (full GPU, no PCIe penalty). tok/s not yet measured. Port: 8098. |
| `qwen2.5-coder:32b` / `qwen3:32b` (dense coder) | 32B dense | **24 GB+** | Coder variants may use more VRAM overhead than the general instruct model — not tested on 20 GB. For reliable headroom on dense 32B coding models, RTX 4090/5090 class (24 GB+) recommended. |
