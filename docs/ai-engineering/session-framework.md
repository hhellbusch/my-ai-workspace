...
**`/checkpoint`** is fast, designed for mid-session use, and built for crash recovery. It writes a minimal snapshot to `.planning/whats-next.md`: what's in progress, what just finished, what the next step is, what decision was made that would otherwise be re-litigated. Intentionally minimal — fast to produce, fast to read. Run it before risky operations (directory moves, large refactors), after completing a significant unit of work, or when more than a few commits have accumulated since the last save. The git log shows what landed; the checkpoint captures what was in flight and what was decided.
