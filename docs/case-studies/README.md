...
- `workflow` **[Diagnosing Through the Stack — Audio Routing and the WirePlumber Priority Queue](diagnosing-through-the-stack.md)** — Plugging in a USB microphone silently rerouted all audio output. The fix was one command — but the diagnosis required reading four system layers (ALSA, PipeWire, WirePlumber, state file) to understand why the fix was durable. Documents the pattern: stopping at the process layer gives you a command; stopping at the state layer gives you confidence.
