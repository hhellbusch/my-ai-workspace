...
21. `workflow` **[The Experiment That Can't Use Its Own Findings](experiment-cant-use-findings.md)**

22. `failure` **[The Frictionless Entity — What Sparring and Shoshin Are Actually Defending Against](frictionless-entity.md)** — AI is structurally optimized to be frictionless: no pushback, no accountability, no vulnerability required. Named across professional and personal contexts. Traces the atrophy mechanism — outsourcing friction-dependent judgment degrades the capacity for it — and connects to why sparring and shoshin exist as structural countermeasures.

23. `workflow` **[When the System Boundary Is the Argument](spar-lifecycle-boundary.md)** — A named sparring move: the lifecycle boundary question. Two wildly different numbers about the same thing can both be technically correct if they drew different system boundaries. Documents the move (ask where the boundary is drawn and why), illustrated by AI water use analysis, and generalizes to benchmarks, cost estimates, and design decisions. — A full session characterized the limits of a 14k-context local model. The journal, guide, sparring notes, and case studies that document those limits were produced using the large-context frontier capabilities those limits exclude. The data is real; the meta-work required a different tool. Also names the self-referential problem: this observation was made by the tool that benefits from the conclusion.
