...
A persistent markdown file with four sections — In Progress, Up Next, Ideas, Done — managed through a [slash command](../../.cursor/commands/backlog.md) with subcommands for adding items, picking work, completing items, reviewing staleness, and prioritizing. Every item has a product tag, context, links, and a date.
...
A [/start command](../../.cursor/commands/start.md) that checks for handoff files, reads the backlog, summarizes recent git activity, checks planning project status, and suggests 2-3 focus options. Complemented by a [session-awareness rule](../../.cursor/rules/session-awareness.md) that passively reminds the AI about persistent context sources (backlog, handoffs, planning, library, git log).
...
A [/review command](../../.cursor/commands/review.md) that acts as a quality gate before commits — checks for broken links, missing cross-references, registry drift, convention violations, and (for essays) an optional "Assumptions to challenge" section. An [always-applied rule](../../.cursor/rules/pre-commit-review.md) reminds the AI to run this before committing.

### 5. Content audit (`/audit`)

A [comprehensive audit command](../../.cursor/commands/audit.md) that systematically checks link integrity, registry alignment (docs/README.md, research/README.md, library/README.md, BACKLOG.md), cross-reference gaps, and content freshness. Read-only — it reports findings and asks what to fix. The first run caught 6 registry drift issues.

### 6. Cross-linking conventions

A [cross-linking rule](../../.cursor/rules/cross-linking.md) defining triggers: new file in docs/ triggers README updates, new research directory triggers research index updates, new library entry triggers catalog updates, renamed files trigger link searches. Plus a [backlog-capture rule](../../.cursor/rules/backlog-capture.md) that reminds the AI to capture deferred ideas immediately rather than losing them when the session ends.
...
This is the meta-development loop from [AI-Assisted Development Workflows](../ai-engineering/ai-assisted-development-workflows.md): notice a gap, build a tool, use the tool, notice the next gap. But here the loop is building organizational infrastructure, not application code. The AI is constructing the system that organizes its own work.
...
This mirrors the problem decomposition pattern from [The Shift](../ai-engineering/the-shift.md) — the human frames the problem, the AI handles implementation — but applied to infrastructure rather than features.
