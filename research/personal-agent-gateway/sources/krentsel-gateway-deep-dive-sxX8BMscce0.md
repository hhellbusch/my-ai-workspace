# Transcript: Principles for Autonomous System Design: Personal Agent Gateway Deep Dive
...
**[0:21]** But the last couple of months we've seen Personal Agent Gateway kind of take off and I got very curious about what makes Personal Agent Gateway work as well as it does.

**[0:31]** So, I've been playing with Personal Agent Gateway for just over a month now and I spent the last couple weeks deep in the code and I put together this talk on the principles for autonomous system design that I've taken away from um just being deep in the code.

**[0:46]** Now, a large part of this talk is me going into the actual architecture of Personal Agent Gateway and what makes it work.
...
**[1:05]** I have about 5 minutes of background. I have probably half an hour or so of me actually going through the Personal Agent Gateway architecture.

**[1:13]** Uh um but then I'm going to also show a little bit of my setup and how I'm using Personal Agent Gateway um and some observations and open discussion questions um that are informing uh my own research now.
...
**[4:01]** And especially Personal Agent Gateway which take this to an even further extreme of being able to modify itself and learn.
...
**[7:28]** And finally, the world we're in now is in a a world of autonomous agents. It's Personal Agent Gateway which has tooling and has full ownership of its environment and it can fully decide um to add more tools, to make changes to itself, to learn in different ways. It kind of owns a broader scope, a wider scope of fully autonomous space as compared to these locally scoped agents.

**[7:47]** Now, I got to ask also, what are people using Personal Agent Gateway for? It's a variety of things. Um I went to visit a friend and I saw that the company he's working at they're using it for product prototyping. People are using it for inbox management, personal assistants.
...
**[8:18]** Now, I just want to highlight the point of this talk is not for me to convince you to use Personal Agent Gateway. Rather, I want us to come out of this talk with an understanding of the principles that uh underlie Personal Agent Gateway's design and what maybe you can take away from for your own system designs.

**[8:34]** But the Personal Agent Gateway value proposition is this.
...
**[8:57]** So, let's dive into the Personal Agent Gateway architecture and see how it looks.

**[9:02]** Now, Personal Agent Gateway itself was released in November of 2025.

**[9:06]** It went viral in 2026.

**[9:08]** Um and I took this tagline here um See if this works. There we go. I took this tagline directly from the Personal Agent Gateway website. This is a screenshot. So, this is in its own in their own words, the creators, what is Personal Agent Gateway? It's the AI that actually does things.

**[9:20]** And I'm going to highlight a few words here that are important. First is AI because obviously under the hood Personal Agent Gateway is calling a large language model that lives somewhere and say hi.

**[9:30]** But there's two other kind of phrases here that are really important.

**[9:34]** The first one is actually doing.

**[9:36]** And so I'm going to claim here, I want to derive out what was the design goal of the Personal Agent Gateway creators?
...
**[9:50]** Um which requires closing the control loop. So, Personal Agent Gateway should kind of view the results of its actions and then make decisions on the next actions that it takes.
...
**[10:43]** So, I claim these are the two Personal Agent Gateway two designs.
...
**[10:55]** But there's three core layers to the Personal Agent Gateway architecture.
...
**[12:34]** So, the code for these connectors, effectively when you go to launch WhatsApp, it asks you for that same um QR code. And then that code pretends to be a uh web client of WhatsApp and sends along that token and fetches new messages for you. So, it mimics being a kind of a legitimate web client for WhatsApp, but actually takes the messages and feeds them into um Personal Agent Gateway.
...
**[13:30]** And this gives Personal Agent Gateway again both more context, but also enables Personal Agent Gateway to act as you. So, to send emails from your email or send texts from your phone number.

**[13:40]** I personally did not do this in my setup because I did not trust Personal Agent Gateway quite that much. So, the other option is to give it its own dedicated phone number and email, which is kind of what I did for my project.

**[13:49]** In my experience.

**[13:51]** Which is safer.

**[13:52]** Now, there's one other uh thing going on here uh is uh there's an Personal Agent Gateway UI that provides an administrative kind of view.
...
**[14:21]** So, a large chunk of the magic of Personal Agent Gateway is in this middle layer, the gateway controller.
...
**[16:05]** I find it's really interesting that in the Personal Agent Gateway architecture, the configuration exists as raw markdown files that are used in agent calls.

**[16:15]** And so there is kind of four of these core files.

**[16:19]** There's a user.md file that has information about the user.

**[16:22]** Um in fact, I will just show what these look like. I pulled this myself from my own Personal Agent Gateway.
...
**[16:46]** So, when Personal Agent Gateway starts, its initial prompt to an LLM and what it goes and decides what to do based off of is this bootstrap.md This is the actual file that I took from uh from the code directly.
...
**[18:00]** Now, the soul is Personal Agent Gateway's kind of attempt at capturing who it is. And it starts with this "You're not a chatbot. You're becoming someone." It's very melodramatic.

**[18:09]** Um but it has all these kind of core truths.

**[18:13]** And what's interesting uh is at the end it specifically says, "This file is yours to evolve. As you learn who you are, update it." And so, Personal Agent Gateway is supposed to actually kind of grow and figure out who it is over time.
...
**[19:06]** Um there's also this agents.md file which uh explains a lot of how kind of to work, reminds the uh Personal Agent Gateway to write things down, store things in memory, gives some security guidelines, um and things to kind of ask about. A lot A lot of the privacy and security stuff is actually just encoded in these text files. So, I imagine it's actually not that hard to trick.
...
**[19:42]** This is tips and tricks for Personal Agent Gateway on how to use certain tools.

**[19:46]** Okay.

**[19:51]** Now, so far we have talked about just this configuration over here.

**[19:55]** I'm going to get now really deep into the core obstruction that personal agent gateway uses which is this idea of sessions.
...
**[20:51]** And this allows the personal agent gateway to schedule for itself things to check in on. It'll say every time you know I'm woken up let me check that this process over here is still running.
...
**[23:10]** And the creators of personal agent gateway just gave personal agent gateway a tool that it can use to schedule cron jobs.

**[23:20]** And again this is just magical because now the agent has specifically the personal agent gateway agent has two ways of interacting with time.
...
**[23:44]** What personal agent gateway will do under the hood is it will say okay let me write up a description of the task.
...
**[24:26]** And so these two things together give personal agent gateway sense of liveliness that is very human-like very autonomous because it can handle both schedule things and unscheduled things.

**[24:37]** There's additionally a memory management module that's a vector database of our past conversations and documents. It also includes the daily summary doc at the end of the day um and this allows personal agent gateway to kind of keep track of context on on different things that it's working on.
...
**[27:11]** For me these things become real when I see the actual tools in the code that are being used and so that's exactly what I wanted to go show. This is a screenshot from the personal agent gateway GitHub that shows the actual set of tools that are built into personal agent gateway. It's the first type of tool that is made available.
...
**[28:37]** But the third thing that that personal agent gateway also has is this generated set of generated LSP tools which give IDE like intelligence. So definition references completion this is language server protocol LSP.
...
**[29:25]** So personal agent gateway generates such tool.
...
**[31:38]** Um Now, in the internals of Personal Agent Gateway, this is all configurable, but by default, you can only have 150 skills or 30,000 characters in the context, in the actual call to the LLM.
...
**[34:12]** Um and over here I have like an actual skill that comes bundled with Personal Agent Gateway.
...
**[36:10]** There's a safety clause here that tries to tell the uh LLM to act kind of safely. That is the ex- almost the extent of security that's built into Personal Agent Gateway. It's not a particularly secure system.
...
**[37:06]** Um couple of other kind of extra information down here. But, this is the core, the entirety of Personal Agent Gateway internals.
...
**[37:23]** Okay. So, at this point, and I'm looking at time, I've done this in the half just under the half an hour that I promised, we have our Personal Agent Gateway architecture. So, we should understand all the boxes on this page now.
...
**[37:54]** Now, Personal Agent Gateway provides the ability to extend functionality. And I would argue this is one of the things that has made made it so successful is uh I've outlined in red here all the different places you can extend it. And the community has extended it a ton.
...
**[38:40]** Even cooler, though, is that uh Personal Agent Gateway has control of these plugins themselves. So, it can go and add its own new new plugins. It can go and fetch and find tools that it needs or fetch and find skills.
...
**[39:11]** Um For setting up connectors, I also interacted entirely through the Personal Agent Gateway UI, telling it what I wanted, and it configured its own plugin for Discord for setting it up, which was really lovely.

**[39:25]** Um one thing it didn't do for me, though it it could. It has access to the terminal, so bash can run commands.

**[39:32]** Um I myself kind of went in and set up the environment in which Personal Agent Gateway was running. And I'll I'll about how to do setup in a minute.
...
**[39:59]** Does Personal Agent Gateway succeed?
...
**[43:54]** Um at first, many people were using kind of iMessage and WhatsApp integrations where you could just text text your uh your Personal Agent Gateway.

**[44:03]** But, think from your Personal Agent Gateway's perspective. In your life, you might have many different projects you want to be working on, or different things.

**[44:09]** Whereas, Personal Agent Gateway kind of sees a particular session, single session, in a connection.
...
**[44:44]** So, to alleviate this, um I use something uh my friend, uh Mehdi Qazi, um one of my closest friends from undergrad, uh at Berkeley together, uh developed uh this kind of way of using uh Personal Agent Gateway is very nice, which is giving it a dedicated Discord server.
...
**[46:00]** Um I think over here, uh I was uh working on a research idea and having uh uh Ludwig, which is the name of my my Personal Agent Gateway agent, work on that.
...
**[46:30]** All of these things you can kind of kick off and work on in parallel. And this allows Personal Agent Gateway to keep separate contexts and keep track of what it's doing and why. It's very useful.
...
**[47:00]** There is environment tooling, which, as I mentioned, is on the server on which Personal Agent Gateway is running, the actual command line tool is available.

**[47:11]** Um for me, this is like the CLI for exc.dev, so I can spin up new VMs, uh Cloud Code. This actually no longer works. You can't use your subscription for Cloud Code, but you can authenticate using an API key, and then uh uh Personal Agent Gateway will go and launch Cloud Code using the API key.
...
**[47:40]** This lets you just authenticate, log in once, and it gives access to all sorts of tooling through Google, including reading Google Docs, uh Google reading and writing Google Docs, Slides, Sheets, Contacts, obviously, of course, emails, your chats, all sorts of other services. Um And it makes uh it actually makes uh your Personal Agent Gateway very powerful.
...
**[48:16]** Um So, Personal Agent Gateway kind of was running some experiments for me and was able to generate some graphs that it produced, and then put them together into a doc, and share that doc with me.
...
**[48:49]** But, Personal Agent Gateway seems very adept at working directly through the CLI.
...
**[50:20]** I think I want to point out I I found myself at first very skeptical about the security story of Personal Agent Gateway.
...
**[50:40]** Um just the same way as you can say Personal Agent Gateway can be tricked, you can also absolutely trick any employee. In fact, that's what phishing emails are. They try to convince an employee to do something by socially engineering them, sending them email email, convincing them to click the link, etc.
...
**[51:09]** Um and I think the Personal Agent Gateway community's bet is that reasoning is getting very close to being good enough to kind of managing its own security by making choices that are kind of reasonable.
...
**[53:09]** Um before even Personal Agent Gateway Code, you could have been talking to ChatGPT and it would tell you what code you need.

**[53:14]** Personal Agent Gateway Code put it in a nice wrapper.
...
**[54:21]** This server is this is not running on the same server as my Personal Agent Gateway. This is running in a separate VM that it figured out how to go and create without me giving it more instructions.
...
**[55:00]** Now, the third topic here, another example I'm very excited about. I decided to push my Personal Agent Gateway to kind of a more extreme point.
...
**[55:20]** This is a uh a YouTube channel that my Personal Agent Gateway created entirely on its own.
...
**[59:08]** So this is the kind of autonomous uh processing that that uh Personal Agent Gateway Code that that my Personal Agent Gateway can do.
...
**[59:33]** In in Personal Agent Gateway, the code powering Personal Agent Gateway.
...
**[1:01:06]** I suspect it's systems that have a malleable architecture. Like, Personal Agent Gateway still has a fixed architecture, which makes it good at particular things. But if even this architecture was something that could self-evolve, now Personal Agent Gateway has the ability to edit its code, and so you could use it to self-evolve.
